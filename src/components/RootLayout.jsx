    ...
    <Container>
      <div className="flex items-center justify-between">
        <Link href={"/"} aria-label="Home">
          <Logo invert={invert}>Studio Clone</Logo>
        </Link>
          ...
          >
            <Icon
              className={clsx(
                "h-6 w-6",
                invert
                  ? "fill-white group-hover:full-neutral-200"
...
const RootLayoutInner = ({ children }) => {
  ...
  useEffect(() => {
    function oc(e) {
      if (e.target.closest("a")?.href === window.location.href) {
        setExpanded(false);
      }
    }
    window.addEventListener("click", oc);
    return () => {
      window.removeEventListener("click", oc);
    };
  }, []);
  return (
    <MotionConfig
      transition={shouldReducedMotion ? { duration: 0 } : undefined}
    >
      <header>
        <div
